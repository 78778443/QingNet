...
class tools:
    ...
    def view_image(img_path, label, position, sort, out_label, out_position, out_sort):
        ...
        if torch.sum(out_position_tensor) != 0:
            cv2.rectangle(new_img, (out_position[0], out_position[1]), (out_position[2], out_position[3]), (0, 0, 255),
                          3)
            cv2.putText(new_img, str(out_sort), (out_position[0], out_position[1] - 3), cv2.FONT_HERSHEY_SIMPLEX, 1,
                        (0, 0, 255), 3)
